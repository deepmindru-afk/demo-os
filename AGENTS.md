...
| `BACKBONE_TIMEOUT` | no | `35` | Per-source ceiling (seconds) for a **backbone** read — the sources that need more than the fast best-effort cap ([`agents/sources.py`](agents/sources.py), `BACKBONE_SOURCES`): the CRM (the rundown's spine) and the workspace (a codebase question is list_files + a couple of file reads). Longer than `PROVIDER_TIMEOUT` so these reliably land while best-effort sources still skip fast; kept under `USE_CONTEXT_TIMEOUT` (minus skill-load + compose headroom) so a rundown still fits the MCP ceiling. |
