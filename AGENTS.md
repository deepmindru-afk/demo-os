...
| `PROVIDER_TIMEOUT` | no | `20` | Per-source ceiling (seconds) for a single `query_<id>` sub-agent run ([`agents/sources.py`](agents/sources.py)). A slow source degrades to a one-line "skipped" and the rest of the brief still lands; smaller than `USE_CONTEXT_TIMEOUT` so several can skip within one budget. |
